...
class BaseCatalogueApplication(Application):
    ...
    def get_urls(self):
        ...
        urlpatterns += [
            ...
            url(r'^(/[\w-]+/)*(?P<product_slug>[\w-]*)_(?P<pk>\d+)/addtofav/$',
                AddProductToFavorite, name='add_to_fav'),
            url(r'^(/[\w-]+/)*(?P<product_slug>[\w-]*)_(?P<pk>\d+)/rmfromfav/$',
                RemoveProductFromFavorite, name='rm_from_fav'),
            url(r'^(/[\w-]+/)*(?P<product_slug>[\w-]*)_(?P<pk>\d+)/oneclick/$',
                OneClickOrderCreateView.as_view(), name='oneclick'),
            url(r'^(?P<category_slug>[\w-]+(/[\w-]+)*)/(?P<product_slug>[\w-]*)_(?P<pk>\d+)/$',
